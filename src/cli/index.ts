...
import { CREATE_INSOMNIAC_NEXTJS_PROJECT, DEFAULT_APP_NAME } from "~/consts.js";
import {
  databaseProviders,
...
export const runCli = async (): Promise<CliResults> => {
  ...
  const program = new Command()
    .name(CREATE_INSOMNIAC_NEXTJS_PROJECT)
    .description("A CLI for creating web applications")
    .argument(
      "[dir]",
    ...
    .option(
      "-y, --default",
      "Bypass the CLI and use all default options to bootstrap a new insomniac-nextjs-app",
      false
    )
    /** START CI-FLAGS */
    /**
  ...
  and likely to result in a crash. Please run create-insomniac-nextjs-app with another
  ...
  can provide the arguments from the CLI directly: https://create.t3.gg/en/installation#experimental-usage to skip the prompts.`);
    ...
    // If the user is not calling create-insomniac-nextjs-app from an interactive terminal, inquirer will throw an IsTTYError
    ...
    if (err instanceof IsTTYError) {
      logger.warn(`
  ${CREATE_INSOMNIAC_NEXTJS_PROJECT} needs an interactive terminal to provide options`);
