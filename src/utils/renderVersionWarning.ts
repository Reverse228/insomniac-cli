...
export const renderVersionWarning = (npmVersion: string) => {
  ...
  if (currentVersion.includes("beta")) {
    logger.warn("  You are using a beta version of create-insomniac-nextjs-app.");
    logger.warn("  Please report any bugs you encounter.");
  } else if (currentVersion.includes("next")) {
    logger.warn(
      "  You are running create-insomniac-nextjs-app with the @next tag which is no longer maintained."
    );
    logger.warn("  Please run the CLI with @latest instead.");
  } else if (currentVersion !== npmVersion) {
    logger.warn("  You are using an outdated version of create-insomniac-nextjs-app.");
    logger.warn(
      "  Your version:",
      currentVersion + ".",
